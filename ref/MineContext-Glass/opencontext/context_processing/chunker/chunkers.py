...
class BaseChunker(ABC):
    ...
    def _create_overlapping_chunks(
        ...
        while start_idx < len(boundaries) - 1:
            ...
            while (chunk_end < len(boundaries) - 1 and 
                   chunk_size + (boundaries[chunk_end + 1] - boundaries[start_idx]) <= max_size):
                chunk_end += 1
            ...
            while (next_start > start_idx and 
...
class StructuredFileChunker(BaseChunker):
    ...
    def _chunk_csv_streaming(self, file_path: Path, context: RawContextProperties) -> Iterator[Chunk]:
    ...
    def _chunk_jsonl_streaming(self, file_path: Path, context: RawContextProperties) -> Iterator[Chunk]:
        ...
        try:
            ...
            for col in df.columns:
                ...
                if 'question' in col_lower or 'q' in col_lower or 'question' in col_lower:
                    question_col = col
                elif 'answer' in col_lower or 'a' in col_lower or 'answer' in col_lower or 'reply' in col_lower:
                    answer_col = col
            ...
            for index, row in df.iterrows():
                ...
                yield Chunk(
                    ...
                    semantic_type="faq",
                    keywords=[word.strip() for word in question.split() if len(word.strip()) > 2][:5],  # First 5 keywords
                    metadata=metadata
                )
