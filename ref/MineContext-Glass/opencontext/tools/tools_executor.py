...
from opencontext.tools.retrieval_tools import (
    ActivityContextTool, IntentContextTool, SemanticContextTool,
    ProceduralContextTool, StateContextTool
)
from opencontext.tools.profile_tools import ProfileEntityTool
from opencontext.tools.operation_tools import (
    GetDailyReportsTool, GetActivitiesTool, GetTipsTool, GetTodosTool
)
...
class ToolsExecutor:
    ...
    async def run_async(self, tool_name: str, tool_input: Dict[str, Any]) -> Any:
        if tool_name in self._tools_map:
            ...
            available_tools_text = f"Available tools: {', '.join(available_tools[:10])}" + ("..." if len(available_tools) > 10 else "")
            return {
                "error": error_msg,
