...
class SmartTipGenerator:
        ...
        try:
            ...
            if activities:
                ...
                for i in range(1, len(activities)):
                    try:
                        prev_time = datetime.datetime.fromisoformat(activities[i-1]['end_time'].replace('Z', '+00:00'))
                        curr_time = datetime.datetime.fromisoformat(activities[i]['start_time'].replace('Z', '+00:00'))
    ...
    def _get_recent_tips(self, days: int = 1) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            recent_tips = self.storage.get_tips(
                start_time=start_time,
                end_time=end_time,
                limit=24
            )
    ...
    def _generate_intelligent_tip_with_patterns(
        ...
        user_prompt = user_prompt_template.format(
            ...
            activity_patterns_info=json.dumps(activity_patterns, ensure_ascii=False, indent=2) if activity_patterns else "{}",
            recent_tips_info=json.dumps(recent_tips, ensure_ascii=False, indent=2) if recent_tips else "[]"
        )
        messages = [
    ...
    def get_recent_tips(self, limit: int = 10) -> List[DocumentData]:
        ...
        try:
            results = self.activity_manager.search_activities(
                query="",
                limit=limit,
                filters={"activity_type": "smart_tip"}
            )
    ...
    def cleanup_old_tips(self, keep_hours: int = 48):
        ...
        try:
            ...
            if deleted_count > 0:
                logger.info(f"Cleaned up {deleted_count} old smart tips over {keep_hours} hours old.")
