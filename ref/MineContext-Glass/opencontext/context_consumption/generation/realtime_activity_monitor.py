...
class RealtimeActivityMonitor:
    ...
    def generate_realtime_activity_summary(
        ...
        try:
            ...
            representative_contexts = self._find_contexts_by_ids(all_context, summary_result.get("representative_context_ids", []))
            resource_data = self._extract_resource_data_from_contexts(representative_contexts, max_count=5)
            ...
            logger.info(f"Real-time activity summary saved to the activity table, ID: {activity_id}")
            return {
                "doc_id": str(activity_id),
                "title": summary_result['title'],
                "content": summary_result['description'],
    ...
    def _generate_concise_summary(
        self, 
        contexts: Dict[str, List[ProcessedContext]],  
        start_time: int, 
        end_time: int
        ...
        try:
            ...
            prompt_group = self.prompt_manager.get_prompt_group("generation.realtime_activity_monitor")
            ...
            for context_type, context_list in contexts.items():
                try:
                    context_data[context_type] = [context.get_llm_context_string() for context in context_list]
                except Exception as e:
            ...
            try:
                ...
                return {
                    ...
                    "representative_context_ids": summary_result.get("representative_context_ids", []),
    ...
    def _find_contexts_by_ids(
        ...
        try:
            ...
            if len(found_contexts) < 5:
                remaining_contexts = [ctx for ctx in contexts if ctx.id not in [fc.id for fc in found_contexts]]
                remaining_contexts.sort(key=lambda x: getattr(x.properties, 'importance', 5), reverse=True)
                ...
                for prop in context.properties.raw_properties:
                    ...
                    if prop.content_format == ContentFormat.IMAGE:
                        ...
                        sources_data.append({
                            "type": "image",
                            "id": prop.object_id,
                            "path": prop.content_path
                        })
                    else:
                        continue
                    sources.add(prop.object_id)      
            except Exception as e:
                logger.debug(f"Failed to extract screenshot from context {getattr(context, 'id', 'unknown')}: {e}")
                continue
        
        logger.info(f"Extracted {len(sources_data)} screenshots from {len(contexts)} representative contexts.")
        return sources_data
